...
| 13 | hx.create | File and Device Management | ESI = Pointer to file name; EDI = Pointer to the content; EAX = File Size | CF defined in case of error or file already present | Save a file on the mounted volume|
| 14 | hx.unlink | File and Device Management | ESI = Pointer to filename | CF defined in case of error or non-existing file | Remove a file on the mounted volume |
| 15 | hx.rename | File and Device management | ESI = Pointer to the original filename; EDI = Pointer to new filename | CF defined in case of error or error in name update | Updates the name of a file on the mounted volume |
| 16 | hx.listFiles | File and Device Management | No input | ESI = Pointer to list of files; EAX = Total Files | Get list of files present on volume |
| 17 | hx.fileExists | File and Device Management | ESI = File name to check | EAX = File Size; CF set if file does not exist | Check if a file exists on the volume |
...
| 69 | hx.changeDirectory | File and Device Management | ESI = Directory name | CF set if the directory is not found or is invalid | Changes the current working directory|
